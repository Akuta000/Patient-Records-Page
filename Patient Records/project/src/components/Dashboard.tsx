...
const Dashboard: React.FC<DashboardProps> = ({ onBackToHome }) => {
  ...
  const calculateStats = (data: PatientRecord[]) => {
    ...
    const otherPatients = data.filter(p => p.gender && p.gender.toLowerCase() !== 'male' && p.gender.toLowerCase() !== 'female').length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-indigo-900 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <InfoTooltip content="Distribution of patients by gender (Male, Female, Other)">
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-xs sm:text-sm text-gray-300 font-medium">Gender Distribution</p>
                <div className="flex items-center space-x-2 mb-2 flex-wrap">
                  ...
                  <span className="text-gray-500">/</span>
                  <span className="text-base sm:text-lg font-bold text-accent-400 font-display">{stats.otherPatients}O</span>
                </div>
                {/* Segmented progress bar */}
                <div className="w-full bg-gray-600 rounded-full h-2 flex overflow-hidden">
                  <div
                    className="bg-primary-500 h-2 transition-all duration-500"
                    style={{ width: `${(stats.malePatients / stats.totalPatients) * 100}%` }}
                  ></div>
                  <div
                    className="bg-secondary-500 h-2 transition-all duration-500"
                    style={{ width: `${(stats.femalePatients / stats.totalPatients) * 100}%` }}
                  ></div>
                  <div
                    className="bg-accent-500 h-2 transition-all duration-500"
                    style={{ width: `${(stats.otherPatients / stats.totalPatients) * 100}%` }}
                  ></div>
                </div>
              </div>
                ...
                <PieChart>
                  <Pie
                    data={diagnosisSummary.map((item, index) => ({
                      ...
                      fill: ['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#D946EF', '#F97316', '#06B6D4', '#84CC16', '#EC4899'][index % 10]
                    }))}
                    cx="50%"
                    cy="50%"
                    labelLine={false}
                    ...
                    dataKey="value"
                  >
                    {diagnosisSummary.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#D946EF', '#F97316', '#06B6D4', '#84CC16', '#EC4899'][index % 10]} />
                    ))}
                  </Pie>
                  <RechartsTooltip
                    contentStyle={{
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-600/50">
                {genderByDiagnosis.map((item, index) => (
